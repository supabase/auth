...
create table if not exists {{ index .Options "Namespace" }}.mfa_recovery_codes(
       ...
       constraint mfa_recovery_codes_user_id_fkey foreign key(factor_id) references {{ index .Options "Namespace" }}.mfa_factors(id) on delete cascade
);
create unique index only_one_recovery_code_per_user on {{ index .Options "Namespace" }}.mfa_factors(user_id,factor_type) where factor_type = 'recovery_code';
