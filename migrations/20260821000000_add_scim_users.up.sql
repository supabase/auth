...
-- user_id is null until the user first signs in and their SAML identity is
-- linked, so it stays nullable for the life of the table.
...
-- externalId is the IdP's own key for the user, so a provider's joiner lookup
-- cannot create duplicates on it either.
...
-- An auth user belongs to one identity provider, so at most one live SCIM
-- record may claim it. Doubles as the reverse lookup from users to scim_users.
