...
func (a *API) Recover(w http.ResponseWriter, r *http.Request) error {
	...
	genericRecoverError := "If a user exists, you will receive an email with instructions on how to reset your password."
	if err != nil {
		if models.IsNotFoundError(err) {
			return notFoundError(err.Error())
		}
		return internalServerError(genericRecoverError).WithInternalError(err)
	}
