...
We support `apple`, `azure`, `bitbucket`, `discord`, `facebook`, `github`, `gitlab`, `google`, `notion`, `spotify`, `slack`, `twitch`, `linkedin` and `twitter` for external authentication.
...
```
provider=apple | azure | bitbucket | discord | facebook | github | gitlab | google | notion | slack | spotify | twitch | twitter
