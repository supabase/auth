...
We support `apple`, `azure`, `bitbucket`, `discord`, `facebook`, `github`, `gitlab`, `google`, `intuit` `keycloak`, `linkedin`, `notion`, `spotify`, `slack`, `twitch`, `twitter` and `workos` for external authentication.
...
```
provider=apple | azure | bitbucket | discord | facebook | github | gitlab | google | intuit | keycloak | linkedin | notion | slack | spotify | twitch | twitter | workos
