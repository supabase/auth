...
func TestServer(t *testing.T) {
	...
	t.Run("Tenant", func(t *testing.T) {
		...
		_, raw := createToken(t, db, tenant)

		served := func(t *testing.T, authorization string) (*httptest.ResponseRecorder, *Tenant) {
			t.Helper()

			var seen *Tenant
			next := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				seen = tenantKey.Value(r.Context())
		...
		t.Run("hands the tenant to the handler", func(t *testing.T) {
			w, seen := served(t, "Bearer "+raw)

			assert.Equal(t, http.StatusTeapot, w.Code)
			assert.Equal(t, tenant, seen)
		})

		t.Run("returns 401 with a challenge when the token is unknown", func(t *testing.T) {
			unknown, _ := models.NewSCIMBearerToken()
		...
		t.Run("returns 401 when token is revoked", func(t *testing.T) {
			token, raw := createToken(t, db, tenant)
			revokedAt := time.Now()
			token.RevokedAt = &revokedAt
			require.NoError(t, db.Update(token))
	...
	create := func(t *testing.T, srv *Server, tenant *Tenant, userName string) *core.User {
		t.Helper()
	...
	t.Run("POST /Users", func(t *testing.T) {
		...
		t.Run("with an oversized request body", func(t *testing.T) {
			...
			r := httptest.
				...
				WithContext(tenantKey.WithValue(t.Context(), tenant))
...
func scimRequest(method, target, body string, tenant *Tenant, params map[string]string) *http.Request {
	...
	return r.WithContext(tenantKey.WithValue(ctx, tenant))
}
