...
func enrollAndVerify(ts *MFATestSuite, user *models.User, token string, guaranteeSuccess bool) (verifyResp *AccessTokenResponse) {
	var buffer bytes.Buffer
	w := httptest.NewRecorder()
...
func (ts *MFATestSuite) TestVerificationHooks() {
	...
	for _, c := range cases {
		ts.T().Run(c.desc, func(t *testing.T) {
			...
			resp := signUpAndVerify(ts, email, password, c.expectToken)
