...
func (ts *MFATestSuite) TestChallengeFactor() {
	...
	token, err := generateAccessToken(ts.API.db, u, nil, time.Second*time.Duration(ts.Config.JWT.Exp), &ts.Config.JWT)
...
func (ts *MFATestSuite) TestMFAVerifyFactor() {
	...
	for _, v := range cases {
		ts.Run(v.desc, func() {
			...
			token, err := generateAccessToken(ts.API.db, user, r.SessionId, time.Second*time.Duration(ts.Config.JWT.Exp), &ts.Config.JWT)
...
func (ts *MFATestSuite) TestUnenrollVerifiedFactor() {
	...
	for _, v := range cases {
		...
		ts.Run(v.desc, func() {
			...
			token, err := generateAccessToken(ts.API.db, u, &s.ID, time.Second*time.Duration(ts.Config.JWT.Exp), &ts.Config.JWT)
			require.NoError(ts.T(), err)
