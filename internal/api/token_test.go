...
// func (ts *TokenTestSuite) TestSignUpPKCE() {
// 	// Sign up with PKCE
// 	var buffer bytes.Buffer
// 	codeVerifier := "4a9505b9-0857-42bb-ab3c-098b4d28ddc2"
// 	codeChallenge := sha256.Sum256([]byte(codeVerifier))
...
// 	u, err = models.FindUserByEmailAndAudience(ts.API.db, "test@example.com", ts.Config.JWT.Aud)
// 	require.NoError(ts.T(), err)
// 	// Email sending will fail and confirmation token is not set so we manually set the token
// 	confirmationToken := "pkce_testconfirmationtoken"
...
// 	// Verify given OTP
// 	requestUrl := fmt.Sprintf("http://localhost/verify?type=%v&token=%v&flow_type=%v", "signup", confirmationToken, models.PKCEFlow.String())
