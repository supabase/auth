...
func (ts *TokenOIDCTestSuite) TestGetProvider() {
	...
	oidcProvider, skipNonceCheck, providerType, acceptableClientIds, allowNoEmail, err := params.getProvider(context.Background(), ts.Config, req)
	require.NoError(ts.T(), err)
	require.NotNil(ts.T(), oidcProvider)
	require.False(ts.T(), skipNonceCheck)
	require.False(ts.T(), allowNoEmail)
