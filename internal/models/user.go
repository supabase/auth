...
func FindSSOUsersByEmailAndProvider(tx *storage.Connection, email, aud, provider string) ([]*User, error) {
	...
	err := tx.Eager().Q().
		...
		Where("id IN (SELECT user_id FROM "+(&Identity{}).TableName()+" WHERE provider = ?)", provider).
		Order("banned_until ASC NULLS FIRST, created_at ASC").
		All(&users)
	if err != nil {
		if errors.Cause(err) == sql.ErrNoRows {
			return nil, nil
...
// FindUsersByProviderWithFilter finds users by provider with optional SCIM filter.
// The filterClause should be generated by ParseSCIMFilterToSQL with SCIMUserFilterAttrs.
// Uses a two-step query to avoid SELECT * on users (which breaks when DB has
// columns not mapped in the Go struct, e.g. is_super_admin).
func FindUsersByProviderWithFilter(tx *storage.Connection, provider string, filterClause *SCIMFilterClause, startIndex, count int) ([]*User, int, error) {
	users := []*User{}
	...
	baseWhere := "i.provider = ? AND u.instance_id = ?"
	baseArgs := []interface{}{provider, uuid.Nil}
	...
	countQuery := `SELECT COUNT(DISTINCT u.id) FROM ` + (&User{}).TableName() + ` u
		INNER JOIN ` + (&Identity{}).TableName() + ` i ON u.id = i.user_id
		WHERE ` + whereClause
	if err := tx.RawQuery(countQuery, args...).First(&totalResults); err != nil {
		return nil, 0, errors.Wrap(err, "error counting users by provider with filter")
	}

	// Get paginated user IDs (GROUP BY deduplicates without DISTINCT+ORDER BY conflict)
	type idRow struct {
		ID uuid.UUID `db:"id"`
	}
	var idResults []idRow
	idQuery := `SELECT u.id FROM ` + (&User{}).TableName() + ` u
		INNER JOIN ` + (&Identity{}).TableName() + ` i ON u.id = i.user_id
		WHERE ` + whereClause + `
		GROUP BY u.id
		ORDER BY u.created_at ASC
		LIMIT ? OFFSET ?`
	idArgs := make([]interface{}, len(args)+2)
	copy(idArgs, args)
	idArgs[len(args)] = count
	idArgs[len(args)+1] = offset
	if err := tx.RawQuery(idQuery, idArgs...).All(&idResults); err != nil {
		if errors.Cause(err) == sql.ErrNoRows {
			return users, totalResults, nil
		}
		return nil, 0, errors.Wrap(err, "error finding user IDs by provider with filter")
	}
	if len(idResults) == 0 {
		return users, totalResults, nil
	}

	// Load full user objects via Pop's query builder (avoids SELECT * on users)
	...
	err := tx.Eager("Identities").Q().
		Where("id IN ("+strings.Join(placeholders, ",")+")", loadArgs...).
		Order("created_at ASC").
		All(&users)
	if err != nil {
		return nil, 0, errors.Wrap(err, "error loading users by provider with filter")
	}
	return users, totalResults, nil
}
